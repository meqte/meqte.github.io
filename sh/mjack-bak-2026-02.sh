...
canshu_v6() {
	if grep -q '^canshu="V6"' /usr/local/bin/m > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/mjack.sh
	fi
}
...
CheckFirstRun_true() {
	if grep -q '^permission_granted="true"' /usr/local/bin/m > /dev/null 2>&1; then
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/mjack.sh
	fi
}
...
send_stats() {
	...
	(
		curl -s -X POST "https://api.Mjack.pro/api/log" \
...
if grep -q '^ENABLE_STATS="false"' /usr/local/bin/m > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/mjack.sh
fi
...
cp -f ./mjack.sh ~/mjack.sh > /dev/null 2>&1
cp -f ~/mjack.sh /usr/local/bin/m > /dev/null 2>&1
ln -sf /usr/local/bin/m /usr/bin/k > /dev/null 2>&1
...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}欢迎使用Mjack脚本工具箱${gl_bai}"
	echo "首次使用脚本，请先阅读并同意用户许可协议。"
	echo "用户许可协议: https://blog.Mjack.pro/user-license-agreement/"
	echo -e "----------------------"
...
install_ldnmp_conf() {
  ...
  sed -i "s#MjackYYDS#$dbusepasswd#g" /home/web/docker-compose.yml
  sed -i "s#Mjack#$dbuse#g" /home/web/docker-compose.yml
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  ...
				  21)
					  ...
					  sed -i "s/Mjack@outlook.com/$cfuser/g" /etc/fail2ban/action.d/cloudflare-docker.conf
...
ldnmp_wp() {
  ...
  mkdir $yuming
  cd $yuming
  wget -O latest.zip ${gh_proxy}github.com/Mjack/Website_source_code/raw/refs/heads/main/wp-latest.zip
  unzip latest.zip
  rm latest.zip
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://blog.Mjack.pro/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示: ${gl_bai}该功能需要root用户才能运行！" && break_end && Mjack
}
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  mkdir $yuming
	  cd $yuming
	  wget -O latest.zip ${gh_proxy}github.com/Mjack/Website_source_code/raw/main/Discuz_X3.5_SC_UTF8_20250901.zip
	  unzip latest.zip
	  rm latest.zip
	  ...
	  cd /home/web/html/$yuming/template/ && wget ${gh_proxy}github.com/Mjack/Website_source_code/raw/main/DYXS2.zip && unzip DYXS2.zip && rm /home/web/html/$yuming/template/DYXS2.zip
	  ...
	  wget ${gh_proxy}github.com/Mjack/Website_source_code/raw/refs/heads/main/ai_prompt_generator.zip
	  unzip $(ls -t *.zip | head -n 1)
	  rm -f $(ls -t *.zip | head -n 1)
...
if [ ! -d apps/.git ]; then
	timeout 10s git clone ${gh_proxy}github.com/Mjack/apps.git
else
	cd apps
	# git pull origin main > /dev/null 2>&1
	timeout 10s git pull ${gh_proxy}github.com/Mjack/apps.git main > /dev/null 2>&1
fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方应用列表"
  	  echo -e "${gl_kjlan}想要让你的应用出现在这里？查看开发者指南: ${gl_huang}https://dev.mjack.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "设置${docker_name}的登录密钥（sk-开头字母和数字组合）如: sk-159Mjackyyds163: " app_passwd
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载Mjack脚本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
		  ...
		  1)
			  while true; do
				  ...
				  if [ "$kuaijiejian" != "k" ]; then
					  ln -sf /usr/local/bin/m /usr/local/bin/$kuaijiejian
				  fi
				  ln -sf /usr/local/bin/m /usr/bin/$kuaijiejian > /dev/null 2>&1
				  echo "快捷键已设置"
				  send_stats "脚本快捷键已设置"
				  break_end
				  linux_Settings
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "请输入新的解析记录 格式: 110.25.5.33 Mjack.pro : " addhost
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "访问Mjack官方留言板，您对脚本有任何想法欢迎留言交流！"
			echo "https://board.Mjack.pro"
			echo "公共密码: mjack.sh"
			  ;;
			...
			while true; do
			  ...
			  case $sub_choice in
				  1)
					  ...
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' ~/mjack.sh
					  echo "已开启采集"
		  ...
		  102)
			  clear
			  send_stats "卸载Mjack脚本"
  echo "卸载Mjack脚本"
  echo "------------------------------------------------"
  echo "将彻底卸载mjack脚本，不影响你其他功能"
  clear
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装Mjack脚本         ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
	  ...
	  case $sub_choice in
		  ...
		  11)
			  local py_task="install_Mjack.py"
			  cluster_python3
			  ;;
		  12)
...
echo "║ ╠═╝║ ║╠╦╝ ║ ║╣   ║╠═╣║  ║  ║╣ ║   ║ ╚═╗ ║"
echo "║ ╩  ╚═╝╩╚═ ╩ ╚═╝  ╩╩ ╩╩═╝╩═╝╚═╝╚═╝ ╩ ╚═╝ ║"
echo "║  ─────── M J A C K  T O O L S ───────   ║"
echo "╚═════════════════════════════════════════╝"
echo " Mjack-Tools v$sh_v | 快捷键: M"
echo -e "${gl_kjlan}--------------------------------${gl_bai}"
...
echo -e "${gl_kjlan}----------------------${gl_bai}"
echo -e "${gl_kjlan}8.   ${gl_bai}测试脚本"
echo -e "${gl_kjlan}----------------------${gl_bai}"
echo -e "${gl_kjlan}9.   ${gl_bai}甲骨文脚本"
echo -e "${gl_kjlan}----------------------${gl_bai}"
echo -e "${gl_kjlan}10.  ${gl_bai}LDNMP建站"
echo -e "${gl_kjlan}----------------------${gl_bai}"
...
echo "以下是k命令参考用例："
echo "启动脚本            m"
echo "安装软件包          m install nano wget | m add nano wget | k 安装 nano wget"
echo "卸载软件包          m remove nano wget | m del nano wget | m uninstall nano wget | m 希载 nano wget"
echo "更新系统            k update | k 更新"
echo "清理系统垃圾        m clean | k 清理"
echo "重装系统面板        m dd | k 重装"
echo "bbr3控制面板        m bbr3 | k bbrv3"
echo "内核调优面板        m nhyh | k 内核优化"
echo "设置虚拟内存        m swap 2048"
echo "设置虚拟时区        m time Asia/Shanghai | m 时区 Asia/Shanghai"
echo "系统回收站          m trash | m hsz | k 回收站"
echo "系统备份功能        m backup | m bf | k 备份"
echo "ssh远程连接工具     m ssh | k 远程连接"
echo "rsync远程同步工具   m rsync | k 远程同步"
echo "硬盘管理工具        m disk | k 硬盘管理"
echo "内网穿透（服务端）  k frps"
echo "内网穿透（客户端）  k frpc"
echo "软件启动            m start sshd | m 启动 sshd "
echo "软件停止            m stop sshd | m 停止 sshd "
echo "软件重启            m restart sshd | m 重启 sshd "
echo "软件状态查看        m status sshd | m 状态 sshd "
echo "软件开机启动        m enable docker | m autostart docke | m 开机启动 docker "
echo "域名证书申请        k ssl"
echo "域名证书到期查询    m ssl ps"
echo "docker管理平面      k docker"
echo "docker环境安装      m docker install |m docker 安装"
echo "docker容器管理      m docker ps |k docker 容器"
echo "docker镜像管理      m docker img |m docker 镜像"
echo "LDNMP站点管理       k web"
echo "LDNMP缓存清理       m web cache"
echo "安装WordPress       m wp |m wordpress |m wp xxx.com"
echo "安装反向代理        m fd |m rp |m 反代 |m fd xxx.com"
echo "安装负载均衡        m loadbalance |k 负载均衡"
echo "安装L4负载均衡      m stream |m L4负载均衡"
echo "防火墙面板          m fhq |m 防火墙"
echo "开放端口            m dkdk 8080 |m 打开端口 8080"
echo "关闭端口            m gbdk 7800 |m 关闭端口 7800"
echo "放行IP              m fxip 127.0.0.0/8 |m 放行IP 127.0.0.0/8"
echo "阻止IP              m zzip 177.5.25.36 |m 阻止IP 177.5.25.36"
echo "命令收藏夹          m fav | m 命令收藏夹"
echo "应用市场管理        k app"
echo "应用编号快捷管理    m app 26 | m app 1panel | m app npm"
echo "fail2ban管理        m fail2ban | m f2b"
echo "显示系统信息        k info"
echo "ROOT密钥管理        m sshkey"
echo "SSH公钥导入(URL)    m sshkey <url>"
echo "SSH公钥导入(GitHub) m sshkey github <user> "
		...
		sshkey)
			...
			case "$1" in
				...
				* )
					echo "错误：未知参数 '$1'"
					echo "用法："
					echo "  m sshkey                  进入交互菜单"
					echo "  m sshkey \"<pubkey>\"     直接导入 SSH 公钥"
					echo "  m sshkey <url>            从 URL 导入 SSH 公钥"
					echo "  m sshkey github <user>    从 GitHub 导入 SSH 公钥"
					;;
			esac
